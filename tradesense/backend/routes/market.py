...
@market_bp.route('/quote-ma', methods=['GET'])
def get_quote_ma():
    symbol = request.args.get('symbol')
    if not symbol:
        return jsonify({'error': 'Symbol required (e.g. IAM, ATW)'}), 400
    
    try:
        data = scrape_morocco_stock(symbol)
        if not data or not data.get('price'):
            raise ValueError("Scraper returned empty data")
        return jsonify(data)
    except Exception as e:
        print(f"Scraper Error for {symbol}: {str(e)}. Using mock data.")
        return jsonify(get_mock_quote(symbol))
